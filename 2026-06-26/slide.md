...
React（`localhost:3000`）と Rails API（`localhost:8000`）を開発中に連携させようとすると CORS エラーが出る。これは意図的な設計で、無関係なサイトが勝手にAPIを叩けないようにするためのブラウザの防衛機構。
...
悪意あるサイトがユーザーのブラウザ経由でAPIを叩ける。  
Cookie や Authorization ヘッダーを使った認証付きリクエストと組み合わさると、  
ユーザーの権限で勝手に操作・情報取得される危険がある（CSRF的な文脈）。

```
悪意あるサイト → ユーザーのブラウザ経由 → 認証済みAPIを叩く → 情報漏洩
```
